...
public class CustomOntology {
	public File getXMLFromOntology(final RelationsInDocument rd)
		...
		for(Triple triple : triples) {
			...
			Resource subject = model.createResource("http://www.semanticweb.org/Relation-extraction/"+triple.getSubjectType()+"#"+triple.getSubject().toLowerCase().replaceAll(" ", "-"));
			subject.addProperty(model.createProperty("http://www.semanticweb.org/Relation-extraction/hasName"), triple.getSubject());
			model.add(node, RDF.subject, subject);
			model.add(node, RDF.predicate, "http://www.semanticweb.org/Relation-extraction/"+triple.getPredicate());
			Resource object = model.createResource("http://www.semanticweb.org/Relation-extraction/"+triple.getObjectType()+"#"+triple.getObject().toLowerCase().replaceAll(" ", "-"));
			object.addProperty(model.createProperty("http://www.semanticweb.org/Relation-extraction/hasName"), triple.getObject());
			model.add(node, RDF.object, object);
			model.add(node, model.createProperty("http://www.semanticweb.org/Relation-extraction/sourceSentence"), triple.getReificationSourceSentence());
			model.add(node, model.createProperty("http://www.semanticweb.org/Relation-extraction/sourceUrl"), triple.getReificationSourceUrl());
		}
